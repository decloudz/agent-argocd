...
install-wfsm:      ## Install workflow server manager (wfsm)
	curl -sSL https://github.com/agntcy/workflow-srv-mgr/releases/download/v0.3.1/wfsm0.3.1_darwin_arm64.tar.gz -o wfsm.tar.gz && mkdir -p ~/.local/bin && tar -xzf wfsm.tar.gz -C ~/.local/bin && rm wfsm.tar.gz && echo "wfsm installed to ~/.local/bin. Make sure this directory is in your PATH."
